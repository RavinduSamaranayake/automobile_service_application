...
// when click the repair button visible the repair modal and get the vehicle id
 ...
 confirmRepair(){
  ...
  axios.post('http://shan-motors.herokuapp.com/api/appointments/book-repair',Repair)
      .then(res=>{
        Alert.alert('Sucess','Your vehicle '+this.state.vehicle_number+` Repair Booking Sucessfully on ` + this.state.repair_date_date +' at '+ this.state.arrival_time ,[{text:'ok'}]);
        this.setState({
          vehicle:'',
          vehicle_number:'',
          service_type:[],
